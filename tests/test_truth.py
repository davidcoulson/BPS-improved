"""Truth marks (truth.py) and the per-tracker blend weight."""
import math
...
def _sample(t, floor="F", vec=None, gain=1.0, x=500.0, y=500.0):
    # Four proxies on a 10 m square (100 px/m) ranging a tracker at (x, y), exactly.
...
def test_buffer_keeps_recent_cycles_per_tracker_and_floor():
...
def test_mark_reference_is_the_median_vector_in_probe_scale():
    mark = {"id": 7, "floor": "F", "x": 1.0, "y": 2.0, "samples": [
        {"gain": 2.0, "tracker_vec": {"a": 4.0, "b": 6.0, "c": 1.0}},
        {"gain": 2.0, "tracker_vec": {"a": 6.0, "b": 6.0}},
        {"gain": 2.0, "tracker_vec": {"a": 5.0, "b": 6.0}},
    ]}
...
def test_summarize_averages_marks_per_tracker():
...
def test_blend_weight_decides_the_estimator_and_is_validated():
    layout = {"tuning": {"position_estimator": "fused"}, "tracker_fp_weights": {"tile": 0.0, "watch": 1.0, "cat": 0.3, "bad": 7}, "tracker_estimators": {"bad": "geometric"}}
    assert sextant._tracker_fp_weight(layout, "cat") == 0.3 and sextant._tracker_fp_weight(layout, "bad") is None
    assert sextant._tracker_estimator(layout, "tile") == "geometric"
    assert sextant._tracker_estimator(layout, "watch") == "fingerprint"
    assert sextant._tracker_estimator(layout, "cat") == "fused"
    assert sextant._tracker_estimator(layout, "bad") == "geometric"      # invalid weight: the override still counts
    assert sextant._fingerprint_wanted({"tuning": {"position_estimator": "geometric"}, "tracker_fp_weights": {"cat": 0.3}})
    db = fp.ReferenceDB()
    saved = sextant._fingerprint_db
    sextant._fingerprint_db = db
    try:
        sextant._seed_tracker_gain({"tracker_fp_gains": {"tile": 1.8}}, "tile")
        assert db.gain_for("tile") == 1.8
        db.tracker_gain["tile"] = 1.2
        sextant._seed_tracker_gain({"tracker_fp_gains": {"tile": 1.8}}, "tile")
