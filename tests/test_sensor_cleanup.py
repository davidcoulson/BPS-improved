...
Before this, untrack only told Bermuda; the four per-tracker sensors and the
...
def _house(tmp_path, trackers=("phone", "tile_1")):
    """A hass whose registry, cache and states carry four sensors per tracker."""
    hass = make_hass(tmp_path)
    hass.states = _States()
    reg, devs = er.async_get(hass), dr.async_get(hass)
    hass.data["sextant_sensors"] = {}
    for tracker in trackers:
        devs.add({("sextant", tracker)}, device_id=f"dev_{tracker}")
        for suffix, _label in sn.SENSOR_KINDS:
            entity_id, unique_id = f"sensor.{tracker}_{suffix}", f"{suffix}_{tracker}"
            reg.add(entity_id, unique_id=unique_id, device_id=f"dev_{tracker}")
...
def _entries_of(hass, tracker):
    return [e for e in er.async_get(hass).entities if e.startswith(f"sensor.{tracker}_")]


def test_tracker_of_unique_id_recognises_every_kind_and_nothing_else():
    assert sn.tracker_of_unique_id("sextant_room_phone") == "phone"
    assert sn.tracker_of_unique_id("sextant_nearest_room_tile_1") == "tile_1"
    assert sn.tracker_of_unique_id("sextant_spot_a_b") == "a_b"
    assert sn.tracker_of_unique_id("sextant_position_accuracy") is None
    assert sn.tracker_of_unique_id(None) is None


def test_remove_sensors_for_trackers_drops_cache_registry_state_and_device(tmp_path):
    ...
    # The other tracker and the global diagnostic are untouched.
    ...
    assert sn.remove_sensors_for_trackers(hass, ["phone", "", None]) == 0
...
def test_device_stays_while_a_foreign_entity_still_lives_on_it(tmp_path):
    ...
    sn.remove_sensors_for_trackers(hass, ["phone"])
